...
3. **Branchless Piecewise Function**: The `Buckingham` kernel uses energy reflection at the potential maximum ($r = r_{max}$) to handle the unphysical divergence at $r \to 0$. This is implemented as a branchless piecewise function: computing both the standard Buckingham energy and the reflected energy ($2E_{max} - E$), then selecting via arithmetic masking. This avoids expensive branch mispredictions and maintains consistent throughput suitable for SIMD vectorization.
